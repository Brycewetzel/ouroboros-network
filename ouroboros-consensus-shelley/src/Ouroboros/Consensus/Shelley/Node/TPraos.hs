...
shelleyBlockForging ::
     forall m era c.
      ( ShelleyCompatible era (TPraos c)
      , PraosCrypto c
      , c ~ EraCrypto era
...
class    (ShelleyCompatible era proto, TxLimits (ShelleyBlock proto era), EraCrypto era ~ c)
  => ShelleyEraWithCrypto c proto era
instance (ShelleyCompatible era proto, TxLimits (ShelleyBlock proto era), EraCrypto era ~ c)
  => ShelleyEraWithCrypto c proto era
...
protocolInfoShelley ::
     forall m c.
      ( IOLike m
      , PraosCrypto c
      , ShelleyCompatible (ShelleyEra c) (TPraos c)
  ...
  where
    ...
    initSnapShot =
        SL.incrementalStakeDistr
          (SL.updateStakeDistribution mempty (SL._utxo (SL._utxoState ledgerState)) mempty)
          dState'
          pState'
