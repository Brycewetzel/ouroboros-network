...
import           Ouroboros.Consensus.Shelley.Protocol.Abstract (ProtoCrypto,
                     ...
                     ShelleyHash (ShelleyHash), ShelleyProtocolHeader)
...
instance PraosCrypto c => ProtocolHeaderSupportsEnvelope (Praos c) where
  ...
  envelopeChecks cfg _lv hdr = do
    ...
    where
      ...
      maxHeaderSize = praosMaxHeaderSize pp
      maxBodySize = praosMaxBodySize pp
...
instance PraosCrypto c => ProtocolHeaderSupportsProtocol (Praos c) where
  ...
  pHeaderIssuer Header {headerBody} = hbVk headerBody
  pHeaderIssueNo Header {headerBody} = SL.ocertN $ hbOCert headerBody
  pHeaderVRFValue Header {headerBody} = certifiedOutput $ hbVrfRes headerBody
